...
import coverImage from '@/assets/images/projects-matias/micelaria/home.png';
import logoImage from '@/assets/images/projects-matias/micelaria/micelaria_logo.png';
import adminImage from '@/assets/images/projects-matias/micelaria/admin.png';
import carrito1Image from '@/assets/images/projects-matias/micelaria/carrito_1.png';
import carrito2Image from '@/assets/images/projects-matias/micelaria/carrito_2.png';
import tiendaImage from '@/assets/images/projects-matias/micelaria/tienda.png';
...
const MicelariaPage: React.FC = () => {
  const { themeMode } = useTheme();
  const { t } = useTranslation();
  const isDark = themeMode === 'dark';

  const highlights = t('micelariaPage.highlights', { returnObjects: true }) as string[];
  const galleryText = t('micelariaPage.gallery', {
    returnObjects: true,
  }) as Record<string, { title: string; description: string }>;

  const galleryItems = useMemo(
    () => [
      { id: 'home', image: coverImage },
      { id: 'tienda', image: tiendaImage },
      { id: 'carrito1', image: carrito1Image },
      { id: 'carrito2', image: carrito2Image },
    ...
    <PageTransition>
      <PageContainer>
        <Header>
          <HeaderContent>
            <LogoRow>
              <LogoImage src={logoImage} alt={t('micelariaPage.logoAlt')} />
            </LogoRow>
            <Title $isDark={isDark}>{t('micelariaPage.title')}</Title>
            <Subtitle $isDark={isDark}>{t('micelariaPage.subtitle')}</Subtitle>
            <Intro $isDark={isDark}>{t('micelariaPage.intro')}</Intro>
            <LinksRow>
              <ExternalLink
                $isDark={isDark}
                href="https://micelaria.com"
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t('micelariaPage.ctaAria')}
              >
                {t('micelariaPage.cta')}
              ...
              <ExternalLink
                $isDark={isDark}
                href="https://www.instagram.com/micelariaok"
                target="_blank"
                rel="noopener noreferrer"
                aria-label={t('micelariaPage.igCtaAria')}
              >
                {t('micelariaPage.igCta')}
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
        ...
        <Section>
          <SectionTitle $isDark={isDark}>{t('micelariaPage.highlightsTitle')}</SectionTitle>
        ...
        <Section>
          <SectionTitle $isDark={isDark}>{t('micelariaPage.stackTitle')}</SectionTitle>
          <TechStack>
            {techStack.map((tech) => (
        ...
        <Section>
          <SectionTitle $isDark={isDark}>{t('micelariaPage.galleryTitle')}</SectionTitle>
          <GalleryGrid>
